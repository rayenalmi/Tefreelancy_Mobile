...
String url = GroupsUtils.BASE_URL+"/jsonnew?idOffer="+idoffer+"&rate="+rate ;
        req.setUrl(url);
        req.setPost(false);
...
//       /portfolioJSON/222/edit?intro="hello%20to%20you"&about=thisistheabout
   ...
   public boolean deleteExp(int id) {
    ...
    public ArrayList<Achievement> getAllAchs() {
        String url = GroupsUtils.BASE_URL + "/achievement/jsongetall";
        req.setUrl(url);
        req.setPost(false);
        req.addResponseListener(new ActionListener<NetworkEvent>() {
            @Override
    ...
    String url = GroupsUtils.BASE_URL + "/achievement/json/f/"+aId+"/";
    req.setUrl(url);
    req.setPost(false);
    req.addResponseListener(new ActionListener<NetworkEvent>() {
        @Override
